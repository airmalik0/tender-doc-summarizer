...
Полный разбор документа занимает 45–75 секунд, повторный запрос того же файла
...
Системный промпт помечен к кэшированию: при разборе многостраничного документа
это снимает повторную оплату одного и того же префикса.
...
2. **Точное вхождение на другой странице.** Модель регулярно ошибается со
   ...
   неправильно: номер страницы исправляется, факт остаётся, в `warnings`
   отмечается число исправлений.
3. **Нечёткое сравнение.** Самый длинный общий кусок задаёт место выравнивания,
   дальше считается, какая доля цитаты присутствует в окне вокруг него кусками не
   короче четырёх символов. Порог — 0.82.

Метрика намеренно **односторонняя**: это полнота цитаты, а не схожесть двух строк.
Обычная схожесть штрафовала бы за вставки в документе, а скобочная расшифровка
суммы посреди предложения там — норма. Фрагменты короче четырёх символов не
считаются, иначе совпадения предлогов и пробелов надули бы оценку любому тексту.

Как порог ведёт себя на практике (замеры на демо-документе):

| Цитата | Оценка | Итог |
|---|---|---|
| Точная | 1.00 | подтверждена |
| С опечатками внутри (след OCR) | 1.00 | подтверждена |
| Потерявшая скобочную вставку | 0.88 | подтверждена |
| Выдуманная сумма | 0.57 | **отклонена** |
| Подмена условия | 0.36 | **отклонена** |
| Пересказ своими словами | 0.13 | **отклонена** |
...
| Файл больше лимита | 413 `file_too_large`, чтение обрывается на превышении |
