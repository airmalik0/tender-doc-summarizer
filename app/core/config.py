...
class Settings(BaseSettings):
    ...
    # Предел на извлечённый текст, а не на размер файла. Страница с потоком,
    # который распаковывается в сотни мегабайт операторов, весит килобайты и
    # проходит любой лимит загрузки — ограничивать надо результат разбора.
    max_document_chars: int = Field(default=1_500_000, ge=1_000)
    # Предел на площадь растра при OCR: MediaBox бывает огромным, и страница
    # в 4000 пунктов при 220 dpi превращается в битмап на 600 МБ.
    max_render_pixels: int = Field(default=40_000_000, ge=1_000_000)
